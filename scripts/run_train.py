...
def main() -> None:
    ...
    elif args.model == "MFN_MACE":
        model = modules.MFN_MACE(
            **model_config,
            correlation=args.correlation,
            gate=modules.gate_dict[args.gate],
            interaction_cls_first=modules.interaction_classes[
    ...
    elif args.model == "ScaleShiftMFN_MACE":
        mean, std = modules.scaling_classes[args.scaling](train_loader, atomic_energies)
        model = modules.ScaleShiftMFN_MACE(
            **model_config,
            correlation=args.correlation,
            gate=modules.gate_dict[args.gate],
            interaction_cls_first=modules.interaction_classes[args.interaction_first],
    ...
    if hasattr(model, "bond_interactions"):
        decay_bond_interactions = {}
        no_decay_bond_interactions = {}
        for name, param in model.bond_interactions.named_parameters():
            if "linear_mid" in name or "conv_tp.weight" in name or "conv_tp_sh.weight" in name:
                decay_bond_interactions[name] = param
            else:
                no_decay_bond_interactions[name] = param
        params_for_dict.append(
            {
                "name": "bond_interactions_decay",
                "params": list(decay_bond_interactions.values()),
                "weight_decay": args.weight_decay,
            }
        )
        params_for_dict.append(
            {
                "name": "bond_interactions_no_decay",
                "params": list(no_decay_bond_interactions.values()),
                "weight_decay": 0.0,
            }
        )
